...
public sealed class AccessService : IAccessService
{
    ...
    private async Task<string?> ValidateAccessAsync(Guid branchId, Customer customer, AccessPass? accessPass, CancellationToken cancellationToken)
    {
        ...
        if (!customer.ActiveMembershipId.HasValue) return "Customer does not have an active plan assigned.";
        if (!customer.HasActiveMembership(DateTimeOffset.UtcNow)) return "Customer plan is expired or not yet active.";
        ...
        if (!membershipIsActive) return "Customer plan is not active.";
